...
type EventsV1alpha1Interface interface {
	...
	ClusterEventBusesGetter
	EventBusesGetter
	EventSourcesGetter
	TriggersGetter
}

// EventsV1alpha1Client is used to interact with features provided by the events.openfunction.io group.
type EventsV1alpha1Client struct {
	restClient rest.Interface
}

func (c *EventsV1alpha1Client) ClusterEventBuses(namespace string) ClusterEventBusInterface {
	return newClusterEventBuses(c, namespace)
}

func (c *EventsV1alpha1Client) EventBuses(namespace string) EventBusInterface {
	return newEventBuses(c, namespace)
}

func (c *EventsV1alpha1Client) EventSources(namespace string) EventSourceInterface {
	return newEventSources(c, namespace)
}

func (c *EventsV1alpha1Client) Triggers(namespace string) TriggerInterface {
...
// NewForConfigAndClient creates a new EventsV1alpha1Client for the given config and http client.
// Note the http client provided takes precedence over the configured transport values.
func NewForConfigAndClient(c *rest.Config, h *http.Client) (*EventsV1alpha1Client, error) {
	config := *c
	if err := setConfigDefaults(&config); err != nil {
		return nil, err
	}
